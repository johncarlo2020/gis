...
<section class="main-section">
        ...
        <div class="container-fluid">
            <table class="table" id="table_id">
                ...
                <tbody>
                    @foreach ($client_classifications as $a)
                        <tr>
...
function classification_reload(){
  ...
  $('#table_id').DataTable({
      ...
      "serverSide": true,
      "ajax": {
        url: "{{ route('classification_reload') }}",
        type: 'POST',
      },
  });
...
$(document).on("click", ".trigger_btn", function () {
    ...
    if (type == 'new') {
            var tmp_body = `
                <div class="form-row">
                    <input type="text" name="name" data-type="validate_new" class="form-control validator" placeholder="Client Classification Name*" required>
            ...
            $(".modal-title").append("Add Client Classification");
    ...
    if (type == 'view') {
        ...
        $.ajax({
            ...
            <div class="form-row mt-2">
                ...
                $(".modal-title").append("View Client Classification");
                $("#modal-form").append(tmp_body);
    ...
    if (type == 'edit') {
        $.ajax({
            ...
            var tmp_body = `
            <div class="form-row">
                    <input type="text" name="name" data-type="validate_edit" data-id="`+data['0'].id+`" class="form-control validator" value="`+data['0'].name+`" placeholder="Client Classification Name*" required>
                    <small id="required-name" class="d-none" style="color:red">Name Required</small>
                    <small id="unique-name" class="d-none" style="color:red">Name Already Exist</small>
                </div>
                <div class="form-row mt-2">
                ...
                $("#modal-form").append(tmp_body); 
                $(".modal-title").append("Edit Client Classification");
    ...
    if (type == 'status') {
        ...
        if (status == 'delete') {
            ...
            $(".modal-title").append("Delete Client Classification");
        ...
        if (status == 'recover') {
            ...
            $(".modal-title").append("Recover Client Classification");
...
$(document).on("click", ".save_btn", function () {
    ...
    if (type == "store") {
            ...
            $.ajax({
                url: "{{ route('classification_store') }}",
                data: {
                    data: data,
                },
                dataType: "json",
    ...
    if (type == "status") {
        $.ajax({
            url: "{{ route('classification_status') }}",
            data: {
                id: id,
            },
            dataType: "json",
...
function validator_scholarship(type) {
    if (type == "validate_new") {
        ...
        $.ajax({
            url: "{{ route('classification_validate') }}",
